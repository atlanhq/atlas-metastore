...
if [ "$1" == "build_without_dashboard" ]; then
 mvn clean -pl '!test-tools,!dashboardv2,!dashboardv3' -Dmaven.test.skip -DskipTests -Drat.skip=true -DskipOverlay -DskipEnunciate=true package -Pdist
else
  mvn clean -pl '!test-tools' -Dmaven.test.skip -DskipTests -Drat.skip=true -DskipEnunciate=true package -Pdist
fi
