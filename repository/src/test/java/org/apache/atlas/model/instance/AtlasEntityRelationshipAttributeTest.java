...
public class AtlasEntityRelationshipAttributeTest {
    ...
    /**
     * Test that when setAddedRelationshipAttribute is called FIRST, then setRemovedRelationshipAttribute
     * is called with the same GUID, the removed is NOT added (skipped).
     * This tests the actual order of operations in the unlink-relink scenario.
     */
    @Test
    public void testSetAddedFirstThenRemovedIsSkipped_MapValue() {
        AtlasEntity entity = new AtlasEntity("TestEntity");

        // First, set the readme as added (simulating re-link that comes first in processing order)
        Map<String, String> addedReadme = new HashMap<>();
        addedReadme.put("guid", README_GUID);
        addedReadme.put("typeName", README_TYPE);
        entity.setAddedRelationshipAttribute(README_ATTR, addedReadme);
        ...
        // Now, try to set the same readme as removed (simulating unlink that comes later in processing order)
        Map<String, String> removedReadme = new HashMap<>();
        removedReadme.put("guid", README_GUID);
        removedReadme.put("typeName", README_TYPE);
        entity.setRemovedRelationshipAttribute(README_ATTR, removedReadme);

        // Verify added is still set
        assertNotNull(entity.getAddedRelationshipAttributes().get(README_ATTR));

        // Verify removed was skipped (NOT added)
        assertTrue(entity.getRemovedRelationshipAttributes() == null ||
                        entity.getRemovedRelationshipAttributes().get(README_ATTR) == null,
                "Same entity should NOT be added to removed when it already exists in added");
    }

    /**
     * Test that when setAddedRelationshipAttribute is called FIRST, then setRemovedRelationshipAttribute
     * is called with the same GUID, the removed is NOT added (skipped) - using AtlasObjectId.
     */
    @Test
    public void testSetAddedFirstThenRemovedIsSkipped_AtlasObjectIdValue() {
        AtlasEntity entity = new AtlasEntity("TestEntity");

        // First, set the readme as added
        AtlasObjectId addedReadme = new AtlasObjectId(README_GUID, README_TYPE);
        entity.setAddedRelationshipAttribute(README_ATTR, addedReadme);
        ...
        // Now, try to set the same readme as removed
        AtlasObjectId removedReadme = new AtlasObjectId(README_GUID, README_TYPE);
        entity.setRemovedRelationshipAttribute(README_ATTR, removedReadme);
    ...
    /**
     * Test that addOrAppendAddedRelationshipAttribute called first, then addOrAppendRemovedRelationshipAttribute
     * with the same GUID - the removed should be skipped.
     */
    @Test
    public void testAddOrAppendAddedFirstThenRemovedIsSkipped() {
        AtlasEntity entity = new AtlasEntity("TestEntity");

        // First, append the readme to added list
        AtlasObjectId addedReadme = new AtlasObjectId(README_GUID, README_TYPE);
        entity.addOrAppendAddedRelationshipAttribute(README_ATTR, addedReadme);
        ...
        // Now, try to append the same readme to removed list
        AtlasObjectId removedReadme = new AtlasObjectId(README_GUID, README_TYPE);
        entity.addOrAppendRemovedRelationshipAttribute(README_ATTR, removedReadme);

        // Verify added list still has the item
        addedValue = entity.getAddedRelationshipAttributes().get(README_ATTR);
        assertEquals(((List<?>) addedValue).size(), 1);

        // Verify removed list was NOT created or the item was skipped
        assertTrue(entity.getRemovedRelationshipAttributes() == null ||
                        entity.getRemovedRelationshipAttributes().get(README_ATTR) == null ||
                        (entity.getRemovedRelationshipAttributes().get(README_ATTR) instanceof List &&
                                ((List<?>) entity.getRemovedRelationshipAttributes().get(README_ATTR)).isEmpty()),
                "Same entity should NOT be added to removed list when it already exists in added list");
    }

    /**
     * Test that different GUIDs are allowed when added is set first, then removed.
     */
    @Test
    public void testDifferentGuidsAllowedWhenAddedFirst() {
        AtlasEntity entity = new AtlasEntity("TestEntity");
        String differentGuid = "different-readme-guid-456";

        // First, set a readme as added
        Map<String, String> addedReadme = new HashMap<>();
        addedReadme.put("guid", README_GUID);
        addedReadme.put("typeName", README_TYPE);
        entity.setAddedRelationshipAttribute(README_ATTR, addedReadme);

        // Now, set a DIFFERENT readme as removed
        Map<String, String> removedReadme = new HashMap<>();
        removedReadme.put("guid", differentGuid);
        removedReadme.put("typeName", README_TYPE);
        entity.setRemovedRelationshipAttribute(README_ATTR, removedReadme);

        // Verify both added and removed have their respective values
        assertNotNull(entity.getAddedRelationshipAttributes().get(README_ATTR));
        assertEquals(((Map<?, ?>) entity.getAddedRelationshipAttributes().get(README_ATTR)).get("guid"), README_GUID);
        assertNotNull(entity.getRemovedRelationshipAttributes().get(README_ATTR),
                "Different guids should be allowed in both added and removed");
        assertEquals(((Map<?, ?>) entity.getRemovedRelationshipAttributes().get(README_ATTR)).get("guid"), differentGuid);
    }
}
