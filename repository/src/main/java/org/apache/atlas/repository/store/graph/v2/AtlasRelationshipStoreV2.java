...
public class AtlasRelationshipStoreV2 implements AtlasRelationshipStore {
    ...
    public AtlasRelationship create(AtlasRelationship relationship) throws AtlasBaseException {
        ...
        if (relationship.getTypeName().equals(UD_RELATIONSHIP_TYPE_NAME)) {
            EntityGraphMapperV3.validateCustomRelationship(end1Vertex, end2Vertex);
        }
    ...
    public AtlasRelationship update(AtlasRelationship relationship) throws AtlasBaseException {
        ...
        AtlasEdge   edge       = graphHelperV3.getEdgeForGUID(guid);
        String      edgeType   = AtlasGraphUtilsV3.getTypeName(edge);
        ...
        if (relationship.getEnd2() != null) {
            ...
            if (updatedEnd2Guid != null) {
                ...
                if (!StringUtils.equalsIgnoreCase(relationship.getEnd2().getGuid(), end2Guid)) {
                    throw new AtlasBaseException(AtlasErrorCode.RELATIONSHIP_UPDATE_END_CHANGE_NOT_ALLOWED, AtlasGraphUtilsV3.getTypeName(edge), guid, end2Guid, relationship.getEnd2().getGuid());
                }
            }
        }
    ...
    private AtlasEdge createRelationship(AtlasVertex end1Vertex, AtlasVertex end2Vertex, AtlasRelationship relationship, boolean existingRelationshipCheck, boolean skipAuth) throws AtlasBaseException {
        ...
        try {
            ...
            if (existingRelationshipCheck) {
                ret = graphHelperV3.getOrCreateEdge(end1Vertex, end2Vertex, relationshipLabel);
            } else {
                ret = graphHelperV3.addEdge(end1Vertex, end2Vertex, relationshipLabel);
                if (LOG.isDebugEnabled()) {
            ...
            if (ret != null) {
                ...
                AtlasGraphUtilsV3.setEncodedProperty(ret, ENTITY_TYPE_PROPERTY_KEY, relationship.getTypeName());
                AtlasGraphUtilsV3.setEncodedProperty(ret, RELATIONSHIP_GUID_PROPERTY_KEY, guid);
                AtlasGraphUtilsV3.setEncodedProperty(ret, HOME_ID_KEY, relationship.getHomeId());
                AtlasGraphUtilsV3.setEncodedProperty(ret, VERSION_PROPERTY_KEY, getRelationshipVersion(relationship));
                AtlasGraphUtilsV3.setEncodedProperty(ret, PROVENANCE_TYPE_KEY, relationship.getProvenanceType());
                AtlasGraphUtilsV3.setEncodedProperty(ret, RELATIONSHIPTYPE_TAG_PROPAGATION_KEY, tagPropagation.name());
            ...
            if (MapUtils.isNotEmpty(relationType.getAllAttributes())) {
                for (AtlasAttribute attr : relationType.getAllAttributes().values()) {
                    ...
                    AtlasGraphUtilsV3.setEncodedProperty(ret, attrVertexProperty, attrValue);
                }
            }
        ...
        if (MapUtils.isNotEmpty(relationType.getAllAttributes())) {
            for (AtlasAttribute attr : relationType.getAllAttributes().values()) {
                ...
                if (relationship.hasAttribute(attrName)) {
                    AtlasGraphUtilsV3.setEncodedProperty(relationshipEdge, attrVertexProperty, relationship.getAttribute(attrName));
                } else {
                    ...
                    if (StringUtils.isNotEmpty(defaultValue)) {
                        ...
                        if (attrValue != null) {
                            AtlasGraphUtilsV3.setEncodedProperty(relationshipEdge, attrVertexProperty, attrValue);
                        }
                    }
                }
            }
    ...
    private void validateRelationship(AtlasVertex end1Vertex, AtlasVertex end2Vertex,  AtlasRelationship relationship) throws AtlasBaseException {
        ...
        String                end1TypeName     = AtlasGraphUtilsV3.getTypeName(end1Vertex);
        String                end2TypeName     = AtlasGraphUtilsV3.getTypeName(end2Vertex);
        ...
        EntityGraphMapperV3.validateCustomRelationshipAttributeValueCase(relAttrs);
    }
    ...
    private void validateEnds(AtlasRelationship relationship) throws AtlasBaseException {
        ...
        for (int i = 0; i < ends.size(); i++) {
            ...
            AtlasVertex         endVertex        = AtlasGraphUtilsV3.findByGuid(this.graph, guid);
            ...
            if (!AtlasTypeUtil.isValidGuid(guid) || endVertex == null) {
                ...
                if (AtlasGraphUtilsV3.findByUniqueAttributes(this.graph, entityType, uniqueAttributes) == null) {
    ...
    public AtlasVertex getVertexFromEndPoint(AtlasObjectId endPoint) {
        ...
        if (StringUtils.isNotEmpty(endPoint.getGuid())) {
            ret = AtlasGraphUtilsV3.findByGuid(this.graph, endPoint.getGuid());
            ...
            ret = AtlasGraphUtilsV3.findByUniqueAttributes(this.graph, entityType, endPoint.getUniqueAttributes());
        }
    ...
    private String getRelationshipEdgeLabel(AtlasVertex fromVertex, AtlasVertex toVertex, String relationshipTypeName) throws AtlasBaseException {
        ...
        AtlasRelationshipEndDef endDef1         = relationshipType.getRelationshipDef().getEndDef1();
        AtlasRelationshipEndDef endDef2         = relationshipType.getRelationshipDef().getEndDef2();
        Set<String>             fromVertexTypes = getTypeAndAllSuperTypes(AtlasGraphUtilsV3.getTypeName(fromVertex));
        Set<String>             toVertexTypes   = getTypeAndAllSuperTypes(AtlasGraphUtilsV3.getTypeName(toVertex));
    ...
    private String getTypeNameFromObjectId(AtlasObjectId objectId) {
        ...
        if (StringUtils.isBlank(typeName)) {
            typeName = AtlasGraphUtilsV3.getTypeNameFromGuid(this.graph, objectId.getGuid());
        }
    ...
    private boolean vertexHasRelationshipWithType(AtlasVertex vertex, String relationshipTypeName) {
        ...
        Iterator<AtlasEdge> iter     = graphHelperV3.getAdjacentEdgesByLabel(vertex, AtlasEdgeDirection.BOTH, relationshipEdgeLabel);
    ...
    public static void recordRelationshipMutation(RelationshipMutation relationshipMutation, AtlasEdge edge, EntityGraphRetrieverV3 entityRetriever) throws AtlasBaseException {
    ...
    private static void setEdgeVertexIdsInContext(AtlasEdge edge) {
        RequestContext.get().addRelationshipEndToVertexIdMapping(GraphHelperV3.getAtlasObjectIdForOutVertex(edge), edge.getOutVertex().getId());
        RequestContext.get().addRelationshipEndToVertexIdMapping(GraphHelperV3.getAtlasObjectIdForInVertex(edge), edge.getInVertex().getId());
    }
