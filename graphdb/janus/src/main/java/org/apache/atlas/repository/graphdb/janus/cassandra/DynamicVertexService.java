...
//    }
    ...
    public void upsertVerticesIfNotNewer(Map<String, Map<String, Object>> allPropertiesMap,
                                         Map<String, Long> updatedAtById) throws AtlasBaseException {
        if (MapUtils.isEmpty(allPropertiesMap)) {
            return;
        }

        Map<String, String> toInsert = new HashMap<>(allPropertiesMap.size());
        Map<String, Instant> updatedAtMap = new HashMap<>(allPropertiesMap.size());

        for (Map.Entry<String, Map<String, Object>> entry : allPropertiesMap.entrySet()) {
            String id = entry.getKey();
            toInsert.put(id, serializer.serialize(entry.getValue()));
            Long updatedAt = updatedAtById != null ? updatedAtById.get(id) : null;
            if (updatedAt != null) {
                updatedAtMap.put(id, Instant.ofEpochMilli(updatedAt));
            }
        }

        repository.upsertVerticesIfNotNewer(toInsert, updatedAtMap);
    }
