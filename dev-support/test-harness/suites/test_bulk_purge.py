...
class BulkPurgeSuite:
    ...
    def test_purge_valid_prefix(self, client, ctx):
        ...
        assert_status_in(resp, [200, 202, 400, 404, 500])
    ...
    def test_system_status_structure(self, client, ctx):
        ...
        if resp.status_code == 200:
            body = resp.json()
            if isinstance(body, dict) and body:
                has_fields = any(k in body for k in ("activeRequests", "queueSize", "status", "requests"))
                assert has_fields or body == {}, (
                    f"System status should have activeRequests/queueSize/status, got keys: {list(body.keys())}"
                )
